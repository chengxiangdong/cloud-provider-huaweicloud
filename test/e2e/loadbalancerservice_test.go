...
var _ = ginkgov2.Describe("loadbalancer service testing", func() {
	...
	ginkgov2.AfterEach(func() {
		...
		if service != nil {
			...
			ginkgov2.By(fmt.Sprintf("Wait for the Service(%s/%s) to be deleted", testNamespace, service.Name), func() {
	...
	ginkgov2.It("service enhanced auto testing", func() {
		...
		var ingress string
		ginkgov2.By("Check service status", func() {
			gomega.Eventually(func(g gomega.Gomega) (bool, error) {
		...
		ginkgov2.By("Check if ELB listener is available", func() {
...
func newSecret(namespace string) *corev1.Secret {
	accessKey := base64.StdEncoding.EncodeToString([]byte(os.Getenv("HC_ACCESS_KEY")))
	secretKey := base64.StdEncoding.EncodeToString([]byte(os.Getenv("HC_SECRET_KEY")))
