...
func CreateDeployment(client kubernetes.Interface, deployment *appsv1.Deployment) {
	ginkgov2.By(fmt.Sprintf("Creating Deployment(%s/%s)", deployment.Namespace, deployment.Name), func() {
...
func RemoveDeployment(client kubernetes.Interface, namespace, name string) {
	ginkgov2.By(fmt.Sprintf("Removing Deployment(%s/%s)", namespace, name), func() {
